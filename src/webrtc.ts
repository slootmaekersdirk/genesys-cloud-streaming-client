...
import {
  ExtendedRTCIceServer,
  IClientOptions,
  SessionTypes,
  IPendingSession,
  StreamingClientExtension,
...
export class WebrtcExtension
  extends EventEmitter
  implements StreamingClientExtension
{
  client: Client;
  ...
  private refreshIceServersRetryPromise:
    | RetryPromise<ExtendedRTCIceServer[]>
    | undefined;
  ...
  async handleStanzaInstanceChange(stanzaInstance: NamedAgent) {
    ...
    this.refreshIceServersTimer = setInterval(
      this.refreshIceServers.bind(this),
      ICE_SERVER_REFRESH_PERIOD
    );
  ...
  prepareSession(options: SessionOpts) {
    ...
    const ignoreHostCandidatesForForceTurnFF = false; //DSL this.getIceTransportPolicy() === 'relay' && isFirefox;
  ...
  addEventListeners() {
    this.client.on("connected", () => {
      ...
      return this.refreshIceServers().catch((error) =>
        this.logger.error(
          "Error fetching ice servers after streaming-client connected",
          {
            error,
  ...
  private async handlePropose(msg: ProposeStanza) {
    ...
    const loggingParams = {
      sessionId: sessionId,
      conversationId: msg.propose.conversationId,
      sessionType,
      isDuplicatePropose,
    };
    ...
    if (sessionInfo.accepted) {
      this.logger.info(
        "proceed already sent for this session, but sending another",
        loggingParams
      );
  ...
  private handleRetract(sessionId: string) {
    this.logger.info(
      "retract received",
      this.getLogDetailsForPendingSessionId(sessionId)
    );
  ...
  async notifyScreenShareStart(
    session: GenesysCloudMediaSession
  ): Promise<void> {
  ...
  async refreshIceServers(): Promise<ExtendedRTCIceServer[]> {
      ...
      this.refreshIceServersRetryPromise = retryPromise<ExtendedRTCIceServer[]>(
        ...
        (error): boolean => {
          if (++this.discoRetries > MAX_DISCO_RETRIES) {
            this.logger.warn(
              "fetching ice servers failed. max retries reached.",
              {
    ...
    return this.refreshIceServersRetryPromise.promise.finally(() => {
      this.discoRetries = 0;
  ...
  async _refreshIceServers(
    ...
    const servicesPromise = new Promise<
      [Required<ExternalServiceList>, Required<ExternalServiceList>]
    >((resolve, reject) => {
      setTimeout(() => {
        reject(new Error("Timeout waiting for refresh ice servers to finish"));
      }, ICE_SERVER_TIMEOUT);

      Promise.all([turnServersPromise, stunServersPromise])
        .then(([turn, stun]) => {
          resolve([turn, stun] as [
            Required<ExternalServiceList>,
            Required<ExternalServiceList>
          ]);
        })
        .catch(reject);
    ...
    const iceServers = [...turnServers.services, ...stunServers.services].map(
    ...
    if (!stunServers.services.length) {
      this.logger.info(
        'No stun servers received, setting iceTransportPolicy to "relay"'
      );
  ...
  getIceTransportPolicy() {
    return this.stanzaInstance?.jingle.config.peerConnectionConfig!
      .iceTransportPolicy;
  }

  setIceTransportPolicy(policy: "relay" | "all", stanzaInstance: NamedAgent) {
    stanzaInstance.jingle.config.peerConnectionConfig!.iceTransportPolicy =
      policy;
  }
