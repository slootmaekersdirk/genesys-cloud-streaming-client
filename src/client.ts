...
export class Client extends EventEmitter {
  ...
  constructor(options: IClientOptions) {
    ...
    this.reconnectOnNoLongerSubscribed =
    ...
    this.logger = new Logger({
      ...
      initializeServerLogging:
        !this.isGuest && !options.optOutOfWebrtcStatsTelemetry,
    ...
    Object.keys(extensions).forEach((extensionName) => {
      const extension: StreamingClientExtension = new extensions[extensionName](
        this,
        options
      );
      ...
      if (typeof extension.on === "function") {
        extension.on(
          "send",
          this.handleSendEventFromExtension.bind(this, extension)
        );
      }
  ...
  private handleSendEventFromExtension(
    extension: StreamingClientExtension,
    data: any,
    message = false
  ) {
    return extension.tokenBucket!.removeTokens(1, () => {
      ...
      if (!stanza) {
        return this.logger.warn(
          "cannot send message, no active stanza client",
          { data, message },
          { skipServer: true }
        );
      }
  ...
  private addInateEventHandlers(stanza: NamedAgent) {
    this.on(
      STANZA_DISCONNECTED,
      this.handleStanzaDisconnectedEvent.bind(this, stanza)
    );
    this.on(
      NO_LONGER_SUBSCRIBED,
      this.handleNoLongerSubscribed.bind(this, stanza)
    );
  ...
  private proxyStanzaEvents(stanza: NamedAgent) {
    ...
    (stanza as unknown as Client).emit = (
      eventName: string,
      ...args: any[]
    ): boolean => {
  ...
  private async handleStanzaDisconnectedEvent(
    disconnectedInstance: NamedAgent
  ): Promise<any> {
    this.logger.info("stanzaDisconnected event received", {
  ...
  private handleNoLongerSubscribed(stanzaInstance: NamedAgent) {
    this.logger.warn("noLongerSubscribed event received", {
      stanzaInstanceId: stanzaInstance.id,
      channelId: stanzaInstance.channelId,
    });
  ...
  async disconnect() {
    ...
    return timeoutPromise(
      ...
      5000,
      "disconnecting streaming service"
    );
  }

  async connect(
    connectOpts: StreamingClientConnectOptions = { keepTryingOnFailure: false }
  ) {
    if (this.connecting) {
    ...
    try {
      ...
      if (!err) {
        error = new Error(
          "Streaming client connection attempted received and undefined error"
        );
  ...
  private backoffConnectRetryHandler(
    connectOpts: StreamingClientConnectOptions,
    err: any,
    ...
    if (!err) {
      additionalErrorDetails.error = new Error(
        "streaming client backoff handler received undefined error"
      );
    } else if (err.name === "AxiosError") {
      const axiosError = err as AxiosError;
      const config = axiosError.config;
      ...
      if ([401, 403].includes(err.response?.status || 0)) {
        this.logger.error(
          "Streaming client received an error that it can't recover from and will not attempt to reconnect",
    ...
    if (err instanceof SaslError) {
      ...
      this.hardReconnectRequired = true;
      Object.assign(additionalErrorDetails, {
        channelId: err.channelId,
        stanzaInstanceId: err.stanzaInstanceId,
      });
    }
    ...
    this.logger.error(
      "Failed streaming client connection attempt, retrying",
      additionalErrorDetails,
      { skipServer: err instanceof OfflineError }
    );
    return true;
  }

  private async makeConnectionAttempt() {
    /* DSL if (!navigator.onLine) {
      throw new OfflineError('Browser is offline, skipping connection attempt');
    }
    */

    await this.prepareForConnect();
    const stanzaInstance =
      await this.connectionManager.getNewStanzaConnection();
    this.connected = true;
    this.connecting = false;
    ...
    if (this.hardReconnectRequired) {
      ...
      if (this.config.jid) {
        ...
        jidPromise = this.http
          .requestApiWithRetry("users/me", jidRequestOpts)
          .promise.then((res) => res.data.chat.jabberId);
      }
      ...
      const channelPromise = this.http
        .requestApiWithRetry(
          "notifications/channels?connectionType=streaming",
          channelRequestOpts
        )
        .promise.then((res) => res.data.id);

      const [jid, channelId] = await Promise.all([jidPromise, channelPromise]);
      this.config.jid = jid;
      this.config.channelId = channelId;
      this.autoReconnect = true;
      this.logger.info("attempting to connect streaming client on channel", {
        channelId,
      });
  ...
  static extend(
    namespace,
    extension: StreamingClientExtension | ((client: Client) => void)
  ) {
    if (extensions[namespace]) {
      throw new Error(
        `Cannot register already existing namespace ${namespace}`
      );
    }
